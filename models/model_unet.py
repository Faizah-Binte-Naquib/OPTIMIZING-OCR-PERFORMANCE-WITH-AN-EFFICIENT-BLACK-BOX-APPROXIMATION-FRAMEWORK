...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        dec1 = nn.functional.interpolate(dec1, scale_factor=2, mode='bilinear', align_corners=True)
    ...
    def _block(in_channels, features, name):
        return nn.Sequential(
            OrderedDict([
                ...
                (name + "norm1", nn.GroupNorm(num_groups=32, num_channels=features)),
                (name + "relu1", nn.ReLU(inplace=True)),
                (name + "conv2", nn.Conv2d(in_channels=features, out_channels=features, kernel_size=3, padding=1, bias=False)),
                (name + "norm2", nn.GroupNorm(num_groups=32, num_channels=features)),
                (name + "relu2", nn.ReLU(inplace=True)),
            ])
        )

    def _resize_and_concat(self, x, enc):
        x = nn.functional.interpolate(x, size=enc.size()[2:], mode='bilinear', align_corners=True)
